...
)
{
    ...
    //set the minimum deltaT to the deltaT fed into the solve function
    scalar dtMin = deltaT;
    //create an instance of a canteraMixture by static casting a thermo().composition() object into the canteraMixture
    const canteraMixture &mix=static_cast<const canteraMixture&>(thermo().composition());
    //make a scalarList that is the size of the number species
    //note that RR_ represents the list of reaction rate per specie originally
    scalarList RRtmp(RR_.size());
    //make a cantera reactor instance
    Reactor react;
    //scalar to represent the number species
    scalar Ns = RR_.size();
    //creating an instance of canteraThermo with the composition stored in mix
    const cnateraThermo &gas1 = mix.cellMixture(0);
    //get molecular weights by using the cantera function???
    ...
    forAll(rho_,cellI)
    {
        try
        {
            //creates a reactor network and adds a reactor to it
            ReactorNet sim;
            sim.addReactor(&react);
            //declares 2 scalarFields the size equal to the number species
            scalarField c0(Ns), c1(Ns);
            //setting the canteraThermo instance gas with species composition data from mix
            const canteraThermo &gas = mix.cellMixture(cellI);
            //sets gas phase concentrations to scalarField c0, cantera function from phase.C
            gas.gas().getConcentrations(c0.begin());
            //puts gas mixture into reactor and sets initial time
            react.insert(gas.gas());
            sim.setInitialTime(0);
            setNumerics(sim);

            //sets time interval to be equal to dT
            ...
            while(timeLeft>SMALL)
            {
                try
                {
                    //sets the max step size to dT
                    sim.integrator().setMaxStepSize(timeLeft);
                    //calls the initialize function from Reactor.cpp(?), since the ReactorNet initialize function does not take parameters, initializes the reactor
                    //sim.initialize(old);
                    //takes a step equal to timeLeft, integrating, returns the time after integrating
                    scalar now = sim.step(timeLeft);
                    scalar dt = now - old;
                    if(old>0){
                        //sets dtMin to be timeLeft-current time if less than dtMin time is left
                        dtMin=min(dt,dtMin);
                    }
                    old = now;
                    timeLeft = deltaT - now;
                }
                //catch cantera calculation error
