    ...
    dcdt_(nSpecie_)
{
    ...
    Info<< "canteraChemistryModel: Number of species = " << nSpecie_
...
) const
{
    ...
    //openfoam function that returns the overall reaction rate for a single reaction I
...
) const
{
    ...
    //openfoam function that returns the species reaction rate for a single reaction I
    //T and P are current conditions, need to find 
    //get cantera to calculate overall reaction rates
    Cantera::IdealGasPhase mixture;

    scalar totalC = 0; //scalar to store the total concentration of gas species in cell
    scalar moleF = 0;  
    ...
    forAll(rho, celli)
    {
        ...
        //all species concentration array
        for (label i=0; i<nSpecie_; i++)
        {
        ...
        //grabs the left hand side and right hand side components along with stoichiometric coefficients and puts them in a string to be put into cantera format
        std::string lhsStr;
        std::string rhsStr;
        std::string moleFrac;
        for (int i = 0; i<R.lhs().size(); i++)
        {
            lhsStr += std::string(R.lhs()[i]) + ":" + R.lhs()[i].stoichCoeff + " ";
        }
        for (int i = 0; i<R.rhs().size(); i++)
        {
            rhsStr += std::string(R.rhs()[i]) + ":" + R.rhs()[i].stoichCoeff + " ";
        }
        for (int i = 0; i<R.species().size(); i++)
        {
            moleF = c_[i]/totalC;
            moleFrac += std::string(R.species()[i]) + ":" + std::string(moleF) + " ";
        }
        //set the temperature and pressure of the mixture to the current cell conditions
        mixture.setState_TPX(Ti, pi, moleFrac);

        //R.lhs() and R.rhs() returns an Openfoam list of the lhs and rhs components
        //species_ is the list of species name in the reaction
        Cantera::Composition lhs = Cantera::parseCompString(lhsStr);
        Cantera::Composition rhs = Cantera::parseCompString(rhsStr);

        //Cantera::Arrhenius cons(readScalar(dict.lookup("A")), readScalar(dict.lookup("beta")), readScalar(dict.lookup("Ta")));
        Cantera::Arrhenius rate(R.A_, R.beta_, R.Ta_);//what units?
        
        std::shared_ptr<Cantera::ElementaryReaction> rxn(lhs, rhs, rate);
        Cantera::GasKinetics kin(mixture, rxn);

        Cantera::vector_fp wdot(kin.nReactions());
        kin.getNetRatesOfProgress(wdot.data());
        //const scalar omegai = wdot[0];
        ...
        RR[celli] = wdot[0]*specieThermo_[si].W();
    }
...
{
    //not sure what this does or where this is
    ...
    //make cantera solution object, make reactor object, make reactornet object
    //pass solution into reactor then to reactornet
    //use reactornet.advance to calculate concentrations
    ...
    forAll(rho, celli)
    {
        ...
        if (Ti > Treact_)
        {
            ...
            //solve function is equal to advance in reactornet.cpp(?)
            ...
            for (label i=0; i<nSpecie_; i++)
            {
                //rate of reaction for individual species RR calculated in calculateRR function
                RR_[i][celli] =
